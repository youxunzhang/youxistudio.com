...
<div class="allgames"> 
    ...
    const sanitizeFileName = (name, fallback = 'file') => {
        ...
        const sanitized = clean(name);
        if (sanitized) {
            return sanitized;
        }

        const fallbackSanitized = clean(fallback);
        return fallbackSanitized || 'file';
    };
    ...
    const parseHtml = () => {
        ...
        parsedGames = nodes.map((node, index) => {
            ...
            const sanitizedName = sanitizeDisplayName(rawName, `游戏_${index + 1}`);
    ...
    const downloadImage = async (game, button) => {
        ...
        try {
            ...
            const filenameBase = game.fileNameBase || game.name;
